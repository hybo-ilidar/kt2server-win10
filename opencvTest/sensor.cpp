...
#if 1
  FILE *fp = fopen("./comnumber.txt", "r");
  if(fp == NULL){
    printf("Make comnumber.txt and write something like this: COM3\n");
    return 0;
  }
  fscanf(fp, "%s", comnumber);
  fclose(fp);
#endif
  
  serialClose();
  
  if(serialOpen((char*)comnumber, 12000000)){
    printf("Serial Open Fail! comport %s\n", comnumber);
    return 0;
  }
...
void *tUdpServer(void *thread_argument) {
  ...
  broadcast = true;
  if( setsockopt( sockfd, SOL_SOCKET, SO_BROADCAST, (char *)&broadcast, sizeof(bool) ) ) {
